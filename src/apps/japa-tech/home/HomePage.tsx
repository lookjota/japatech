...
function WhatsAppCta({ children, message, className = '' }: { children: string; message: string; className?: string }) {
  ...
  if (!href) return null
  return <Button className={className} href={href} target="_blank" rel="noreferrer" onClick={() => trackEvent('whatsapp_click', { source: 'home' })}>{children}</Button>
}
