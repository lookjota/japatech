...
function Shell() { return <div className="japa-app"><a className="skip-link" href="#conteudo-principal">Ir para o conteúdo principal</a><Header /><Analytics /><Routes><Route path="/__design-system" element={<TechnicalPreview />} />{publicRoutes.map(({ pathname, page }) => <Route key={pathname} path={pathname} element={<PageView page={page} />} />)}<Route path="*" element={<PageView page={notFoundPage} />} /></Routes><Footer /></div> }
