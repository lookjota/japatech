...
function Shell() { return <div className="japa-app"><a className="skip-link" href="#conteudo-principal">Ir para o conteúdo principal</a><Header /><Analytics /><Routes><Route path="/__design-system" element={<TechnicalPreview />} /><Route path="/" element={<HomePage page={homePage} />} /><Route path="/assistencia-tecnica" element={<AssistanceHubPage page={publicRoutes.find(({ pathname }) => pathname === '/assistencia-tecnica')!.page} />} />{publicRoutes.filter(({ pathname }) => pathname !== '/' && pathname !== '/assistencia-tecnica' && pathname !== '/assistencia-tecnica/tv').map(({ pathname, page }) => <Route key={pathname} path={pathname} element={<PageView page={page} />} />)}<Route path="/assistencia-tecnica/tv" element={<TvLandingPage page={publicRoutes.find(({ pathname }) => pathname === '/assistencia-tecnica/tv')!.page} />} /><Route path="*" element={<PageView page={notFoundPage} />} /></Routes><Footer /></div> }
