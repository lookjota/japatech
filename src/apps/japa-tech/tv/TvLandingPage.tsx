...
function TvWhatsAppCta({ children, message }: { children: string; message: string }) {
  ...
  if (!href) return null
  return <Button href={href} target="_blank" rel="noreferrer" onClick={() => trackEvent('whatsapp_click', { source: 'tv-landing' })}>{children}</Button>
}
...
export function TvLandingPage({ page }: { page: Page }) {
  return <main className="tv-page" id="conteudo-principal">
    ...
    <Section className="tv-home-service" aria-labelledby="tv-home-service-title"><ScrollReveal className="tv-home-service__grid"><TvMediaPlaceholder label="ESPAÇO RESERVADO PARA CENA DE ATENDIMENTO" /><Surface tone="dark"><p className="eyebrow">ATENDIMENTO EM DOMICÍLIO</p><h2 id="tv-home-service-title">Dependendo do defeito, podemos ir até você.</h2><p>Você informa o problema e, a partir das informações iniciais, verificamos a melhor forma de atendimento. Em alguns casos, o atendimento pode acontecer na residência; em outros, a TV pode precisar de avaliação ou reparo em oficina.</p><TvWhatsAppCta message="Olá, Japa Tech. Preciso de assistência para minha TV. Marca/modelo: ____. Problema: ____. ">Falar sobre minha TV</TvWhatsAppCta></Surface></ScrollReveal></Section>
